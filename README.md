...
Folhas de pagamento → PDF / ZIP → Processamento em Python → Base Excel estruturada

Durante o processamento são realizadas etapas como:

- Identificação da estrutura do documento
- Extração dos dados
...
**Processo manual → Excel → Power Query → Python → Processamento de PDFs → FastAPI + Interface Web → Aplicação Desktop → Executável Windows (.exe)**
